...
export default function CornerAvatar() {
  ...
  return (
    <div
      className="fixed bottom-0 left-0 z-40 pointer-events-none flex items-end p-3 sm:p-5"
    ...
    >
      <AnimatePresence>
        {bubble && (
          <motion.div
            ...
            className="pointer-events-none absolute bottom-[calc(100%-0.5rem)] left-4 sm:left-6 max-w-[min(240px,calc(100vw-7rem))] mb-2"
          >
      ...
      <motion.button
        type="button"
        onClick={handleClick}
        className="corner-avatar-btn pointer-events-auto relative shrink-0 focus:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-background rounded-2xl"
        aria-label="Say hi to Rishabh"
        title="Say hi!"
        initial={{ y: 120, opacity: 0 }}
        animate={{ y: 0, opacity: 1 }}
        transition={{ type: 'spring', damping: 16, stiffness: 140, delay: 0.6 }}
        whileHover={{ scale: 1.04, y: -4 }}
        whileTap={{ scale: 0.98 }}
      >
        <motion.img
          src={mascotAvatarUrl}
          alt="Rishabh — your marketing analyst guide"
          className="corner-avatar-img w-[clamp(88px,22vw,148px)] h-auto object-contain object-bottom drop-shadow-2xl"
          draggable={false}
          animate={{ y: [0, -5, 0] }}
